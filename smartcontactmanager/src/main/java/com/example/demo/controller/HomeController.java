...
{
	model.addAttribute("title", "Home- Smart ContactManager");
	return "home";
}
...
{
	model.addAttribute("title", "About- Smart ContactManager");
	return "about";
}
...
{
	model.addAttribute("Register", "About- Smart ContactManager");
...
public String registerUser(@Valid @ModelAttribute("user") User user, BindingResult results, @RequestParam(value ="agreement", defaultValue = "false") boolean agreement,Model model,HttpSession session)
{
	try {
		if(!agreement)
		{
			...
			throw new Exception("You have not agreed the terms and the conditions");
		}
